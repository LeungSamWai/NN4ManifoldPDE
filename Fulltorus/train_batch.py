...
parser.add_argument('--trial', type=int, help='trial num')
# Miscs
...
class FcNet_activate(nn.Module):
    ...
    def forward(self, x):
        x = (x - args.left) * 2 / (args.right - args.left) + (-1)
...
def main():
    ...
    # training_set = Dataset(args.width)
    ...
    if args.optim == 'SGD':
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay = args.weight_decay)#1e-5)
    else:
    ...
    for epoch in range(args.epoch):
        length = x.size(0)
        rnd = np.random.choice(np.arange(length), size=length//10, replace=False, p=None)
        Lsub, fcsub = torch.cuda.FloatTensor(L[rnd]), fc[rnd]
        batch = (Lsub, fcsub)
        print(Lsub.size(), fcsub.size())
...
def train(batch, x, u_true, model, criterion, optimizer, use_cuda, iter, lr):
    ...
    with torch.no_grad():
        ...
        suffix = suffix + 'MSE GT: {diff:.4f}, Abs {abs:.4f}| Integral: {int:.4f}|'.format(diff = MSE, int = integral, abs = torch.max(torch.abs(u_nn - u_true)))
    ...
    '''
    L, fc = batch
    # print(L.size()) # bs * 2500
    # print(fc.size()) # bs * 1
    # print(x.size()) # bs*3
    # L, fc = L.float().cuda(), fc.float().cuda()

    u = model(x)
    # print(torch.max(x, dim = 0), torch.min(x, dim = 0))
    embedded_dev = torch.mm(L, u)
    # print(embedded_dev.size())
    # print(fc.size())
    loss = criterion(embedded_dev, fc)# /torch.mean(outputs**2) #+ bd_loss
    if args.penalty>0:
        # print('penalty used')
    ...
    suffix = 'iter: {iter:.0f}|lr: {lr:.4f}| Batch: {bt:.2f}s | Sum: {sum:.8f} | MSE: {mse: .5f} |'.format(
        bt=batch_time, iter=iter, lr=lr, sum=torch.sum(torch.abs(u)).item(), mse=loss.item()) + suffix
